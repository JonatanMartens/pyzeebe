...
class ZeebeJobAdapter(ZeebeAdapterBase):
    ...
    async def fail_job(self, job_key: int, message: str) -> FailJobResponse:
        try:
            return await self._gateway_stub.FailJob(FailJobRequest(jobKey=job_key, errorMessage=message))
        except grpc.aio.AioRpcError as rpc_error:
            if self.is_error_status(rpc_error, grpc.StatusCode.NOT_FOUND):
                raise JobNotFoundError(job_key=job_key)
            elif self.is_error_status(rpc_error, grpc.StatusCode.FAILED_PRECONDITION):
    ...
    async def throw_error(self, job_key: int, message: str) -> ThrowErrorResponse:
        try:
