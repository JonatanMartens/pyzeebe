...
class Job(object):
    ...
    async def set_failure_status(self, message: str) -> None:
        ...
        """
        if self.zeebe_adapter:
            await self.zeebe_adapter.fail_job(job_key=self.key, message=message)
        else:
            raise NoZeebeAdapterError()

    async def set_error_status(self, message: str) -> None:
        """
        ...
        """
        if self.zeebe_adapter:
            await self.zeebe_adapter.throw_error(job_key=self.key, message=message)
        else:
            raise NoZeebeAdapterError()
